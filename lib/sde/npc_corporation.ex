defmodule SDE.NPCCorporation do
  ...
  defmodule Division do
    @corp_divisions SDE.BSD.load!("crpNPCCorporationDivisions.yaml") |> SDE.BSD.to_map("corporationID")

    def list(corporation_id), do: Map.get(@corp_divisions, corporation_id)
  end

  defmodule ResearchField do
    @corp_research_fields SDE.BSD.load!("crpNPCCorporationResearchFields.yaml") |> Enum.group_by(& &1["corporationID"], & &1["skillID"])

    def list(corporation_id), do: Map.get(@corp_research_fields, corporation_id)
  end

  defmodule Trades do
    @corp_trades SDE.BSD.load!("crpNPCCorporationTrades.yaml") |> Enum.group_by(& &1["corporationID"], & &1["typeID"])
